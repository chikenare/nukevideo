...
`config:clear` first, always: `TestCase` drops and re-migrates the database, and a cached
config makes it read the **development** connection instead of the test one, wiping the real
MariaDB. This is what `composer test` does too.
...
Report the pass/fail counts. On failure, show the failing test names and assertion output —
do not summarize a failure as a success, and do not fix anything unless asked.
