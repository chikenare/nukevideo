...
class GenerateVideoStoryboard implements ShouldQueue
{
    ...
    private function generateSpriteImage(
        ...
        $spriteLocalPath = Storage::disk('tmp')->path("{$video->ulid}_{$spriteName}");
        ...
        Storage::put("{$video->ulid}/{$spriteName}", file_get_contents($spriteLocalPath));
        @unlink($spriteLocalPath);
    }
