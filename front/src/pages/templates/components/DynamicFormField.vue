...
<template>
  <div class="space-y-2">
    ...
    <Input v-else-if="parameter.inputType === 'integer'" :id="paramKey" type="number" :min="parameter.min"
      :max="parameter.max" :model-value="typeof value === 'boolean' ? '' : (value ?? '')" @update:model-value="handleNumberInput"
      :placeholder="parameter.placeholder" />

    <!-- Ktext input (text with validation) -->
    <Input v-else-if="parameter.inputType === 'text'" :id="paramKey" type="text" :model-value="typeof value === 'boolean' ? '' : (value ?? '')"
